...
import { ActivityIndicator, FlatList, Text, View, Image, Button ,TouchableOpacity,Alert} from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import Ipcim from './Ipcim';

const NintendoJatekok = ({navigation}) => {
  const [isLoading, setLoading] = useState(true);
  const [data, setData] = useState([]);

  const getMovies = async () => {
    try {
      const response = await fetch(`${Ipcim.Ipcim}NintendoJatekok`);
      ...
      <View style={{ flex: 1, padding: 24,marginTop:-20,marginBottom:-20}}>
        ...
        {isLoading ? (
          ...
          <FlatList
            ...
            <TouchableOpacity style={{ backgroundColor: "#06c995", width: 175, height: 45, padding: 8, marginLeft: 82, borderRadius: 5 }} onPress={() => navigation.navigate('Játék_Nintendo', {id:item.jatekok_id,nev:item.jatekok_nev, kep:item.jatekok_kep, ar:item.jatekok_ar, ev:item.jatekok_megjelenes, trailer:item.jatekok_trailer })}>
...
export default NintendoJatekok;